...
function nestByDot(mappings) {
  ...
  for (const m of mappings) {
    ...
    if (parts.length === 2) {
      (tree[parts[0]] || (tree[parts[0]] = Object.create(null)))[parts[1]] = m.value;
    } else {
  ...
  switch (format) {
    case "toml": {
      ...
      for (const section of Object.keys(tree)) {
        ...
        if (leaves && typeof leaves === "object" && !normHex(String(leaves))) {
 ...
 */
function convert({ sourceEntry, colors, clientEntry, themeName, minConfidence }) {
